--[[ ASTA REMAKE – Full Hub (Simplificado) Speed Booster (LinearVelocity) integrado ao Carry Speed e Lagger. Botões móveis individuais + fundo personalizado. Botão de Lock na barra superior. Reset aprimorado (substitui Insta Reset e Medusa Reset). Bypass TP + Anti Die. Drop com ApplyImpulse. SEM AIMBOT. ]]

local Players = game:GetService("Players") local TweenService = game:GetService("TweenService") local UIS = game:GetService("UserInputService") local RunService = game:GetService("RunService") local Lighting = game:GetService("Lighting") local HS = game:GetService("HttpService") local player = Players.LocalPlayer

-- EARLY CONFIG LOAD

local introSoundEnabled = true if isfile and isfile("AstaRemake.json") then local ok, data = pcall(function() return HS:JSONDecode(readfile("AstaRemake.json")) end) if ok and type(data) == "table" and data.introSoundEnabled ~= nil then introSoundEnabled = data.introSoundEnabled end end if isfile and isfile("AstaRemake.json") then local ok2, d2 = pcall(function() return HS:JSONDecode(readfile("AstaRemake.json")) end) if ok2 and type(d2)=="table" then if type(d2.animEnabled)=="boolean" then animEnabled=d2.animEnabled end if type(d2.backgroundEnabled)=="boolean" then backgroundEnabled=d2.backgroundEnabled end if type(d2.backgroundIndex)=="number" then backgroundIndex=d2.backgroundIndex end end end

-- INTRO SOUND

local introSoundInstance = nil if introSoundEnabled then local urlIntro = "https://files.catbox.moe/hg5cr4.mp3" local numeFisier = "movee_intro.mp3" local ok, data = pcall(function() return game:HttpGet(urlIntro) end) if ok and data then pcall(function() writefile(numeFisier, data) end) end introSoundInstance = Instance.new("Sound") pcall(function() introSoundInstance.SoundId = getcustomasset(numeFisier) introSoundInstance.Volume = 3 introSoundInstance.Looped = false introSoundInstance.Parent = game:GetService("CoreGui") introSoundInstance:Play() end) end

repeat task.wait() until game:IsLoaded()

-- ============================================================ -- SKY THEME SYSTEM -- ============================================================ local CANDY_SKY_TAG = "AstaSkyTheme" local currentSkyTheme = "Night" local CANDY_SKY_PRESETS = {
    ["Off"]={kind="off"},
...
local function CandyApplyCustomSky(mode)
    ...
    if not preset or preset.kind=="off" then
        Lighting.ClockTime=14;Lighting.Brightness=2;Lighting.OutdoorAmbient=Color3.fromRGB(127,127,127);Lighting.Ambient=Color3.fromRGB(127,127,127);Lighting.FogEnd=100000;Lighting.GlobalShadows=true;return
    end
...
-- ============================================================ -- STATE -- ============================================================ local TS=TweenService local LP=Players.LocalPlayer local NS,CS=59,29 local LAGGER_SPEED = 15 -- Única velocidade para o Lagger (antes era LAGGER_CARRY_SPEED) local carrySpeedActive = false local laggerModeEnabled = false

-- ============================================================ -- SPEED BOOSTER (LinearVelocity) – CORRIGIDO PARA NÃO TRAVAR -- ============================================================ local linearVelocity = nil local lvAttachment = nil local savedWalkSpeed = NS local speedBoosterConn = nil

-- Função auxiliar para obter a velocidade efetiva do booster local function getEffectiveBoosterSpeed()
    if laggerModeEnabled then return LAGGER_SPEED
    elseif carrySpeedActive then return CS
    else return NS -- fallback (não deve ser usado)
    end
end
...
local function setupLV(hrp)
    ...
    -- Usar VectorVelocity para permitir movimento livre em todas as direções
...
local function removeSpeedBoost()
    if linearVelocity and linearVelocity.Parent then linearVelocity.VectorVelocity = Vector3.zero end
    destroyLV()
    local hum = LP.Character and LP.Character:FindFirstChildOfClass("Humanoid")
    if hum then hum.WalkSpeed = savedWalkSpeed end
end

-- Loop do booster (agora com VectorVelocity) local function startSpeedBoosterLoop()
...
-- Função centralizada para atualizar o booster local function updateBooster()
    local shouldActivate = carrySpeedActive or laggerModeEnabled
...
-- ============================================================ -- NOVA FUNÇÃO DROP (com ApplyImpulse) -- ============================================================ local dropEnabled = true local dropActive = false local DROP_ASCEND_DURATION = 0.2 local DROP_ASCEND_SPEED = 150
...
function runDrop()
    if not dropEnabled then print("Drop está desativado.") return end
    if dropActive then print("Drop já em execução.") return end
...
-- ============================================================ -- SISTEMA DE RESET (substitui Insta Reset e Medusa Reset) – MAIS RÁPIDO -- ============================================================ local resetCooldown = false local resetThread = nil local currentCharacter = nil local resetSuccessful = false local stopResetSequence = false local RESET_MAX_DURATION = 0.025 -- reduzido de 0.05 para 0.025
...
LP.CharacterAdded:Connect(function() StopResetSequence() resetCooldown = false currentCharacter = nil resetSuccessful = false stopResetSequence = false end)
...
-- Keybind para reset (padrão: R) local resetKeybind = Enum.KeyCode.R -- Carregar keybind salvo (se houver) if isfile and isfile("AstaRemake.json") then local ok, d = pcall(function() return HS:JSONDecode(readfile("AstaRemake.json")) end) if ok and type(d)=="table" and type(d.resetKeybind)=="string" then local ok2, kc = pcall(function() return Enum.KeyCode[d.resetKeybind] end) if ok2 and kc and kc ~= Enum.KeyCode.Unknown then resetKeybind = kc end end end

UIS.InputBegan:Connect(function(inp, gp)
    if gp then return end
    if UIS:GetFocusedTextBox() then return end
    if inp.KeyCode == resetKeybind then ResetPlayer() end
end)

-- ============================================================ -- ===== NOVO ANTI-RAGDOLL INÍCIO =====
...
-- ===== NOVO ANTI-RAGDOLL FIM =====

-- ============================================================ -- ANTIRAGDOLL, INFJUMP, ETC -- ============================================================ local antiRagdollEnabled,infJumpEnabled=false,false local medusaCounterEnabled,batCounterEnabled,unwalkEnabled=false,false,false local medusaDebounce,medusaLastUsed=false,0 local ASTA={antiKick=false} -- removido medusaReset local autoLeftEnabled,autoRightEnabled=false,false local autoLeftSetVisual,autoRightSetVisual=nil,nil local speedLabel=nil local autoSwingEnabled=true local autoMoveSwingEnabled=false local autoMoveSwingInterval=0.3 local _alSwingDebounce=false local _arSwingDebounce=false local setBatCounterVisual=nil local startBatCounter,stopBatCounter local antiLagEnabled,removeAccessoriesEnabled,antiLagDescConn=false,false,nil local stretchRezEnabled,stretchRezConn,setStretchRezVisual=false,nil,nil local unwalkSavedAnimate,_anyKeyListening=nil,false local autoTPEnabled,autoTPHeight,autoTPConn,setAutoTPVisual=false,20,nil,nil local guiTransparencyEnabled,mobileButtonsEnabled,mobileButtonsLocked=false,true,false local mobileButtonsSize=80 local circleButtonsEnabled=false local stealBarFrame local mobBtnRefs={} local mobGuiRef=nil local fovValue=80 local fovOptions={80,120,180} local fovIndex=1 local laggerModePillRef=nil local carryModePillRef=nil local autoSwitchSpeedEnabled=false local mobBtnTransparencyEnabled=false local perButtonDragEnabled=true local brainrotDetected=false local activeBatBillboard=nil local activeMedusaBillboard=nil local ragdollGuiEnabled=true local persistentRagdollGui=nil local uiLocked=false local infJumpMode="manual" local holdInfJumpConn=nil local _GuiKeys = nil

-- ============================================================ -- BYPASS TP (substitui o TP Lock) -- ============================================================ local bypassTPEnabled = false local bypassTPConn = nil local bypassHitCD = false local bypassSwingCD = 0.2 local bypassHitDist = 6 local bypassSetVisual = nil
...
function stopBypassTP()
    ...
    local hum = char and char:FindFirstChildOfClass("Humanoid")
    if hum then hum.AutoRotate = true pcall(function() hum:ChangeState(Enum.HumanoidStateType.GettingUp) end) end
    bypassHitCD = false
end
...
-- ============================================================ -- ANTI DIE -- ============================================================ local antiDieEnabled = true local antiDieHumanoid = nil local antiDieSetVisual = nil
...
local function setupAntiDie(char)
    ...
    antiDieHumanoid.Died:Connect(function() if antiDieEnabled then healPlayer() end end)
end
...
-- ============================================================ -- CYBER EXTRAS (background + animações) -- ============================================================ local animEnabled = false local backgroundEnabled = false local backgroundIndex = 0 local bgImageRef = nil
...
-- ANIMATII REMBEMBI local RembembiAnims = {
    WalkAnim = 73718308412641,
    RunAnim = 135515454967877,
...
-- ============================================================ -- MOBILE BUTTONS – INDIVIDUAIS ARRASTÁVEIS (sem aimbot) -- ============================================================ local MOB_POS_FILE = "AstaBtnPos.json"
...
-- ============================================================ -- FUNÇÕES EXISTENTES (anti-ragdoll, steal, etc) -- ============================================================ local function addShimmerToLabel(lbl,color1,color2)
...
local function createRagdollBillboard(duration,labelText,color)
    ...
    task.spawn(function()
        local t=0;while timerLbl and timerLbl.Parent do t=t+0.04;shimmer.Offset=Vector2.new(math.sin(t)*0.5,0);task.wait(0.04) end
    end)
...
local function setupSpeedIndicator(char)
    ...
    task.spawn(function()
        local t=0;while bb and bb.Parent do t=t+0.03;gr1.Offset=Vector2.new(math.sin(t)*0.4,0);gr2.Offset=Vector2.new(math.sin(t)*0.4,0);task.wait(0.04) end
    end)
end
...
local Conns={autoSteal=nil,batCounter=nil,anchor={}} -- antiRag removido
    ...
    for _,plot in ipairs(plots:GetChildren()) do
        ...
        for _,podium in ipairs(podiums:GetChildren()) do
            ...
            for _,obj in ipairs(att:GetChildren()) do
                if obj:IsA("ProximityPrompt") and obj.Enabled then return true,d end
            end
        end
    end
...
startAutoLeft=function()
    ...
    alConn=RunService.Heartbeat:Connect(function()
        ...
        if autoMoveSwingEnabled and not _alSwingDebounce then _alSwingDebounce=true task.delay(autoMoveSwingInterval,function() _alSwingDebounce=false end) end
    end)
end
startAutoRight=function()
    ...
    arConn=RunService.Heartbeat:Connect(function()
        ...
        if autoMoveSwingEnabled and not _arSwingDebounce then _arSwingDebounce=true task.delay(autoMoveSwingInterval,function() _arSwingDebounce=false end) end
    end)
end
...
local function doAutoTPDown(force)
    ...
    if not force then
        if hum2.FloorMaterial~=Enum.Material.Air then return end;if not(hrp.Position.Y>=autoTPHeight) then return end
    end
...
enableStretchRez=function()
    ...
    pcall(function()
        RunService:BindToRenderStep(STRETCH_NAME,Enum.RenderPriority.Last.Value-1,function()
            local cam=workspace.CurrentCamera;if cam then cam.CFrame=cam.CFrame*CFrame.new(0,0,0,1,0,0,0,0.8,0,0,0,1) end
        end)
    end)
end
...
disableAntiLag=function()
    ...
    pcall(function()
        if defLightBrightness then Lighting.Brightness=defLightBrightness end;if defLightClock then Lighting.ClockTime=defLightClock end;if defLightAmbient then Lighting.OutdoorAmbient=defLightAmbient end;Lighting.ExposureCompensation=0
    end)
end
...
-- ============================================================ -- ANTI KICK (Asta) -- ============================================================ function ASTA.enableAntiKick()
    ASTA.antiKick=true
    task.spawn(function()
...
function ASTA.disableAntiKick() ASTA.antiKick=false;brainrotDetected=false end
...
startBatCounter=function()
    ...
    Conns.batCounter=RunService.Heartbeat:Connect(function()
        ...
        if st==Enum.HumanoidStateType.Physics or st==Enum.HumanoidStateType.Ragdoll or st==Enum.HumanoidStateType.FallingDown then
            batCounterDebounce=true;task.spawn(function()
                local bat=findBatForCounter();if bat then swingBatForCounter(bat,char) end;task.wait(0.5);batCounterDebounce=false
            end)
        end
    end)
end
...
local function resetAllSettings()
    NS=59;CS=29;LAGGER_SPEED=15;carrySpeedActive=false;laggerModeEnabled=false
...
refreshSpeedModeLabel=function()
    if modeValLbl then
        if laggerModeEnabled then modeValLbl.Text = "Lagger"
...
toggleLaggerMode=function()
    if not laggerModeEnabled then
        ...
        if carrySpeedActive then carrySpeedActive = false end
        updateBooster()
    else
...
-- ============================================================ -- STEAL BAR -- ============================================================ local function createStealBar()
...
-- ============================================================ -- FULL CONFIG LOAD (incluindo resetKeybind, dropEnabled etc) -- ============================================================ pcall(function()
...
-- ============================================================ -- GUI PRINCIPAL (ASTA REMAKE) -- ============================================================ ;(function()
...
local C={
    ...
    bgDark=Color3.fromRGB(3,3,3),
    row=Color3.fromRGB(16,16,16),
    input=Color3.fromRGB(16,16,16),
    blue=Color3.fromRGB(210,210,210),
    blueDim=Color3.fromRGB(70,70,70),
    blueDark=Color3.fromRGB(22,22,22),
...
local function startKL(lbl,onSet)
    ...
    local cap=lbl; task.delay(8,function()
        if KeyListen.label==cap and KeyListen.active then
            cancelKL(); if lbl and lbl.Parent then lbl.Text=prettyKey(Keys.guiHide); lbl.BackgroundColor3=C.blue; lbl.BackgroundTransparency=0.5 end
        end
    end)
end
UIS.InputBegan:Connect(function(inp,gp)
...
local function addInputRow(parent,label,value,order,cb)
    ...
    Box.FocusLost:Connect(function()
        local n=tonumber(Box.Text); if n and n>0 then cb(n) else Box.Text=tostring(value) end
    end)
...
local function addToggleRow(parent,label,enabled,order,kbKey,onToggle)
    ...
    Track.BackgroundColor3=Color3.fromRGB(35,35,40); Track.BackgroundTransparency=0.3
...
-- PÁGINAS
-- SPEED PAGE (removido "Lagger Normal")
;(function()
    ...
    addInputRow(sp,"Lagger Speed",LAGGER_SPEED,4,function(v) LAGGER_SPEED=v; saveConfig() end)
    addSectLbl(sp,"CONTROLS",5)
    addToggleRow(sp,"Carry Mode (Booster)",carrySpeedActive,6,"carryMode",function(on)
        carrySpeedActive = on
        updateBooster()
        if mobBtnRefs.carrySpeed then mobBtnRefs.carrySpeed(carrySpeedActive) end
        if refreshSpeedModeLabel then refreshSpeedModeLabel() end
    ...
    addToggleRow(sp,"Lagger Mode",laggerModeEnabled,7,"laggerToggle",function(on)
        laggerModeEnabled=on
        if on and carrySpeedActive then carrySpeedActive=false updateBooster()
        elseif not on then carrySpeedActive = _prevCarryBeforeLagger or false updateBooster() end
        if mobBtnRefs.lagger then mobBtnRefs.lagger(on) end
        if refreshSpeedModeLabel then refreshSpeedModeLabel() end; saveConfig()
    end)
end)()

-- COMBAT PAGE (sem aimbot)
;(function()
    ...
    local _,svAntiKick=addToggleRow(cp,"Anti Kick",ASTA.antiKick,5,nil,function(on)
        if on then ASTA.antiKick=false; ASTA.enableAntiKick() else ASTA.disableAntiKick() end saveConfig()
    end)
    ...
    -- Toggle para ativar/desativar o Drop
    local _, svDropEnabled = addToggleRow(cp, "Drop Enabled", dropEnabled, 10, nil, function(on)
        dropEnabled = on
        saveConfig()
    end)
end)()
...
;(function()
    ...
    local _,svAutoLeft=addToggleRow(mv,"Auto Left",autoLeftEnabled,1,"autoLeft",function(on)
        if on then
            if autoRightEnabled then autoRightEnabled=false;stopAutoRight();if autoRightSetVisual then autoRightSetVisual(false) end;if mobBtnRefs.autoRight then mobBtnRefs.autoRight(false) end
    ...
    local _,svAutoRight=addToggleRow(mv,"Auto Right",autoRightEnabled,2,"autoRight",function(on)
        if on then
            if autoLeftEnabled then autoLeftEnabled=false;stopAutoLeft();if autoLeftSetVisual then autoLeftSetVisual(false) end;if mobBtnRefs.autoLeft then mobBtnRefs.autoLeft(false) end
    ...
    local _,svAutoTP=addToggleRow(mv,"Auto TP",autoTPEnabled,4,nil,function(on)
...
;(function()
    ...
    -- BYPASS TP TOGGLE (corrigido – cores escuras)
    ...
    -- LOCK UI TOGGLE
    addToggleRow(vi,"Lock UI (freeze drag)",uiLocked,16,nil,function(on)
        uiLocked = on
        if lockBtn then
            lockBtn.Text = uiLocked and "🔒" or "🔓"
            lockBtn.BackgroundColor3 = uiLocked and Color3.fromRGB(60,200,80) or Color3.fromRGB(50,50,50)
...
UIS.InputBegan:Connect(function(inp,gp)
    ...
    if inp.KeyCode==Keys.guiHide then
        if GuiRefs.outer then GuiRefs.outer.Visible=not GuiRefs.outer.Visible end
    elseif inp.KeyCode==Keys.speed then
        speedToggleAction(); saveConfig()
    elseif inp.KeyCode==Keys.carryMode then
        toggleCarryMode(); saveConfig()
    elseif inp.KeyCode==Keys.laggerToggle then
        toggleLaggerMode(); saveConfig()
    elseif inp.KeyCode==Keys.reset then
        ResetPlayer()
    elseif inp.KeyCode==Keys.tpDown then
        runTPFloor()
...
print("ASTA REMAKE LOADED – Lagger corrigido e simplificado, Reset mais rápido, movimento livre no Lagger.")
